...
// @grant        unsafeWindow
// @updatedAT    Sun, 19 Mar 2023 23:06:47 GMT
// ==/UserScript==
...
(function (CryptoJS, $, bootstrap, lodash, sprintfJs) {
  'use strict';
...
`;
  ...
  const unsafeWindowC = _unsafeWindow;
...
`;
    ...
    showOutputOnTextarea(fileName, out) {
      ...
      $(".sak32009 textarea#sake_output").val(out).scrollTop(0);
    }
    setEvents() {
      ...
      $sakeSelect.on("change", (event) => {
        ...
        if (typeof selectedOption === "string") {
          ...
          if (selectedOption === "greenLuma2020ManagerBlueAmulet" || selectedOption === "steamLauncherMiniOnlyDlcsList" || selectedOption === "smokeApiOnlyDlcsList") {
            out = out.replaceAll(/,(?=\s*[}\]])/gu, "");
            const outParse = JSON.parse(out);
            switch (selectedOption) {
              case "greenLuma2020ManagerBlueAmulet": {
              ...
              case "steamLauncherMiniOnlyDlcsList": {
              ...
              case "smokeApiOnlyDlcsList": {
                Object.keys(
                  outParse[this.extractedData.appId].dlcs
                ).forEach((key) => {
                  const appName = outParse[this.extractedData.appId].dlcs[key];
                  outParse[this.extractedData.appId].dlcs[key] = lodash.unescape(appName);
                });
                break;
              }
            }
