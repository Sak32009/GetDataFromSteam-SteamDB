...
// use vite-plugin-monkey@2.3.0 at 2022-09-02T21:54:58.048Z
...
[/dlcs]
`}}},S="sak32009-gaxvyvrguokgtog",h="Get Data from Steam / SteamDB",O="4.5.3";var f=(A=Reflect.get(document,"__monkeyWindow"))!=null?A:window;f.GM,f.unsafeWindow=(C=f.unsafeWindow)!=null?C:window;var v=f.unsafeWindow;f.GM_info,f.GM_cookie;const X="",x=v,U=(typeof x.jQuery>"u"?x.wrappedJSObject:x).jQuery;class Q{constructor(){T(this,"extractedData",{appId:"",name:"",countAllDlcs:0,dlcs:{},countDlcs:0,dlcsUnknowns:{},countDlcsUnknowns:0});T(this,"options",{withDlcsUnknowns:!1});T(this,"is")}check(){this.run();let{href:e}=window.location;$("a[href]").on("click",()=>{const{href:t}=window.location;e!==t&&(e=t,this.run(!0))})}run(e=!1){e&&$(".sak32009").remove();const{href:t}=window.location,s=new URL(t).searchParams;/https:\/\/steamdb\.info\/app\/\d+\/dlc\//u.test(t)?(this.is="steamdbapp",this.getDataFromSteamDBApp()):/https:\/\/steamdb\.info\/app\/\d+\/depots\//u.test(t)?s.get("branch")==="public"&&(this.is="steamdbacf",this.getDataFromSteamDBForACF()):/https:\/\/steamdb\.info\/depot\/\d+\//u.test(t)?s.has("show_hashes")&&(this.is="steamdbdepot",this.getDataFromSteamDBDepot()):/https:\/\/store\.steampowered\.com\/app\/\d+\/\w+/u.test(t)&&(this.is="steampowered",this.getDataFromSteamPowered())}getDataFromSteamDBApp(){this.extractedData.appId=$("div[data-appid]").attr("data-appid"),this.extractedData.name=$('h1[itemprop="name"]').text().trim(),$("#dlc.tab-pane tr.app[data-appid]").each((e,t)=>{const s=$(t),n=s.attr("data-appid"),a=s.find("td:nth-of-type(2)"),d=a.text().trim();a.hasClass("muted")?(this.extractedData.dlcsUnknowns[n]=d,this.extractedData.countDlcsUnknowns+=1):(this.extractedData.dlcs[n]=d,this.extractedData.countDlcs+=1),this.extractedData.countAllDlcs+=1}),this.extractedData.countAllDlcs>0&&this.setModal()}getDataFromSteamPowered(){this.extractedData.appId=$("div[data-appid]").attr("data-appid"),this.extractedData.name=$("div#appHubAppName").text().trim(),$("a.game_area_dlc_row").each((e,t)=>{const s=$(t),n=s.attr("data-ds-appid"),a=s.find(".game_area_dlc_name").text().trim();this.extractedData.dlcs[n]=a,this.extractedData.countDlcs+=1,this.extractedData.countAllDlcs+=1}),this.extractedData.countAllDlcs>0&&this.setModal()}getDataFromSteamDBDepot(){const e=$("div[data-depotid]").attr("data-depotid"),t=U("#files.tab-pane .table.file-tree").DataTable().data().toArray();let s="";for(const n of t){const a=n,d=a[0],i=a[1];i!=="NULL"&&(s+=`${i} *${d}
`)}s.length>0&&(this.setModal(),this.showOutputOnTextarea(`${e}.sha1`,s))}getDataFromSteamDBForACF(){const e=Number($("div[data-appid]").attr("data-appid")),t=$('h1[itemprop="name"]').text().trim(),s=$('#config.tab-pane > table td:first-child:contains("installdir")').closest("tr").find("td:last-child").text().trim(),n=Number($('#depots.tab-pane > ul.app-json i:contains("buildid")').closest("li").find("b").text().trim());console.log("appId",e),console.log("appName",t),console.log("appInstallDirectory",s),console.log("appBuildId",n);const a={};a[e]={common:{name:t},config:{installdir:s},depots:{branches:{public:{buildid:n}}}},$("#depots.tab-pane > .table-responsive").first().find("tr").each((d,i)=>{const o=$(i),M=Number(o.find("td:nth-child(1) a").text().trim()),L=o.find("td:nth-child(2)").text().trim(),c=o.find("td:nth-child(3)").attr("data-sort"),r=typeof c<"u"?Number(c):0,w=o.find("td:nth-child(4)").attr("data-sort"),j=o.find("td:nth-child(5) a").text().trim(),D=o.find("td:nth-child(6)").text();a[e].depots[M]={name:L,maxsize:r},typeof w<"u"&&(a[e].depots[M].config={oslist:w}),j.length>0&&(a[e].depots[M].manifests={public:Number(j)});const y=/DLC (?<dlcid>\d+)/u.exec(D);y!==null&&(a[e].depots[M].dlcappid=Number(y[1]));const I=D.includes("Shared Install");if(I){const g=/Depot from (?<depotid>\d+)/u.exec(D);g!==null&&(a[e].depots[M].sharedinstall=1,a[e].depots[M].depotfromapp=Number(g[1]))}console.log("-------------------------- depotId",M),console.log("depotName",L),console.log("$depotSize",c),console.log("depotSize",r),console.log("depotOs",w),console.log("depotManifestId",j),console.log("depotExtraInfo",D),console.log("depotIsDlc",y),console.log("depotIsSharedInstall",I)}),this.setModal(),this.showOutputOnTextarea(`appmanifest_${e}.acf`,b(e,a))}setModal(){this.setModalContainer(),this.is!=="steamdbdepot"&&this.is!=="steamdbacf"&&this.setEvents(),this.setModalButton()}setModalButton(){const e=`<div class='sak32009'>
    <button
      type='button'
      ...
      data-bs-target='#${S}'
    >
      <img src='${E}' alt='${h} Main' style='width: 30px; height: auto;' />
      <span class='ms-1'>${h} v${O}</span>
    </button>
  </div>`;$(e).appendTo("body")}setModalContainer(){const e=this.is==="steamdbapp",t=this.is==="steamdbdepot",s=this.is==="steamdbacf",n=this.is==="steampowered";let a="";$.each(z,(M,L)=>{a+=`<option value='${M}'>${L.name}</option>
`});let d=`<div class='input-group p-1 bg-white border-top border-1 border-sake-secondary'>
    <select id='sake_select' class='form-select border-sake-secondary rounded-0'>
      ${a}
    </select>`;e&&(d+=`<label class='btn btn-outline-sake-secondary' for='sake_unknowns'>
      <div class='form-check'>
        <input class='form-check-input' type='checkbox' id='sake_unknowns' />
        <span>With DLCS Unknowns</span>
      </div>
    </label>`),d+=`<a href='#' id='sake_download' class='btn btn-outline-sake-secondary rounded-0'>Download as file</a>
  </div>
  ...
  <div class='d-flex flex-row justify-content-end p-2 text-bg-sake-secondary'>
    ...
    <div class='me-1'>|</div>`,e&&(d+=`<div class='me-1'>DLCs Unknowns: ${this.extractedData.countDlcsUnknowns}</div>
      <div class='me-1'>|</div>`),d+=`<div>Total DLCs: ${this.extractedData.countAllDlcs}</div>
  </div>`;const i=`<div class='d-flex justify-content-end p-1 bg-white border-top border-1 border-sake-secondary'>
    <a href='#' id='sake_download' class='btn btn-outline-sake-secondary rounded-0'>Download as file</a>
  </div>
  <pre id='sake_output' class='bg-white text-dark p-2 mb-0 border-top border-1 border-sake-secondary'></pre>
  `;let o=`<div class='sak32009'>
      ...
      <div class='modal-dialog modal-dialog-centered modal-lg'>
        <div class='modal-content text-bg-sake-primary'>
          <div class='modal-header flex-column border-0 text-center'>
            <div>
              <img class='modal-header-logo' src='${Y}' alt='${h} Author' />
            </div>
            <h5>${h} v${O}</h5>
            <div class='flex-row'>
          ...
          <div class='modal-body p-0'>`;e||n?o+=d:(s||t)&&(o+=i),o+=`</div>
  ...
  </div>`,$(o).appendTo("body")}showOutputOnTextarea(e,t){$(".sak32009 a#sake_download").attr({download:e,href:this.encodeToDataUri(t)}),$(".sak32009 pre#sake_output").html(t).scrollTop(0)}setEvents(){const e=".sak32009 select#sake_select";$(e).on("change",t=>{t.preventDefault();const s=$(t.currentTarget).find(":selected").val();if(typeof s=="string"){const n=z[s].file,a=n.text,d=this.parse(n.name);let i=this.parse(a);s==="greenLuma2020ManagerBlueAmulet"&&(i=JSON.stringify(JSON.parse(i.replace(/,\]/gu,"]")),void 0,4)),this.showOutputOnTextarea(d,i)}}),$(e).trigger("change"),$(".sak32009 input#sake_unknowns").on("change",t=>{this.options.withDlcsUnknowns=$(t.currentTarget).is(":checked"),$(e).trigger("change")})}encodeToDataUri(e){const t=$("<textarea>").html(e)[0].value,s=CryptoJS.enc.Utf8.parse(t);return`data:text/plain;charset=utf-8;base64,${CryptoJS.enc.Base64.stringify(s)}`}parse(e){return e=e.replace(/\[dlcs(?: (?<fromZero>fromZero))?(?: prefix="(?<prefix>\d*)")?\](?<content>[\s\S]+?)\[\/dlcs\]/gu,(t,s,n,a)=>{const d=typeof s>"u"?1:0,i=typeof n>"u"?0:Number(n),o=this.options.withDlcsUnknowns?{...this.extractedData.dlcs,...this.extractedData.dlcsUnknowns}:this.extractedData.dlcs;let M=d,L="";for(const c in o)if(Object.prototype.hasOwnProperty.call(o,c)){const r=o[c],w=this.prefixDlcIndex(M.toString(),i);L+=a.replace(/\{(?<content>\w+)\}/gu,(j,D)=>({dlcId:c,dlcIndex:w,dlcName:r})[D]),M+=1}return L}),e=e.replace(/\[data\](?<data>[\s\S]*)\[\/data\]/gu,(t,s)=>this.extractedData[s]),e}prefixDlcIndex(e,t){return t>e.length?"0".repeat(t-e.length)+e:e}}new Q().check()})();
