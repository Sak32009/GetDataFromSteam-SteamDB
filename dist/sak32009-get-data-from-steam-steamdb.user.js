...
// @require      https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.2.0/js/bootstrap.bundle.min.js
// @require      https://cdnjs.cloudflare.com/ajax/libs/crypto-js/4.1.1/crypto-js.min.js
// @grant        unsafeWindow
// ==/UserScript==

// use vite-plugin-monkey@2.3.0 at 2022-09-03T11:19:03.937Z
...
[dlcs]{dlcId} = {dlcName}
[/dlcs]
`}},creamApi3410:{name:"CreamAPI v3.4.1.0",file:{name:"cream_api.ini",text:`[steam]
; Application ID (http://store.steampowered.com/app/%appid%/)
appid = [data]appId[/data]
; Current game language.
; Uncomment this option to turn it on.
...
`}},greenLuma2020BatchMode:{name:"GreenLuma 2020 [BATCH MODE]",file:{name:"[data]appId[/data]_GreenLuma_2020.bat",text:`@ECHO OFF
...
:EXIT
EXIT
`}},greenLuma2020ManagerBlueAmulet:{name:"GreenLuma 2020 Manager [BlueAmulet]",file:{name:"[data]appId[/data]_GreenLuma_2020_Manager_BlueAmulet.json",text:`[[dlcs]{"id":"{dlcId}","name":"{dlcName}","type":"DLC"},[/dlcs]]
`}},lumaEmuOnlyDlcsList:{name:"LUMAEMU (ONLY DLCS LIST)",file:{name:"[data]appId[/data]_lumaemu.ini",text:`[dlcs]; {dlcName}
DLC_{dlcId} = 1
[/dlcs]
`}},codexDlc00000DlcName:{name:"CODEX (DLC00000 = DLCName)",file:{name:"[data]appId[/data]_codex.ini",text:`[dlcs prefix="5"]DLC{dlcIndex} = {dlcId}
DLCName{dlcIndex} = {dlcName}
[/dlcs]
`}},threeDmGameOnlyDlcsList:{name:"3DMGAME (ONLY DLCS LIST)",file:{name:"[data]appId[/data]_3dmgame.ini",text:`[dlcs fromZero prefix="3"]; {dlcName}
DLC{dlcIndex} = {dlcId}
[/dlcs]
`}},skidrowOnlyDlcsList:{name:"SKIDROW (ONLY DLCS LIST)",file:{name:"[data]appId[/data]_skidrow.ini",text:`[dlcs]; {dlcName}
{dlcId}
[/dlcs]
`}},appIdAppIdName:{name:"APPID = APPIDNAME",file:{name:"[data]appId[/data]_appid_appidname.ini",text:`[dlcs]{dlcId} = {dlcName}
[/dlcs]
`}},appIdName:{name:"APPIDNAME",file:{name:"[data]appId[/data]_appidname.ini",text:`[dlcs]{dlcName}
[/dlcs]
`}}},C="sak32009-gaxvyvrguokgtog",x="Get Data from Steam / SteamDB",k="4.5.4";var I=(b=Reflect.get(document,"__monkeyWindow"))!=null?b:window;I.GM,I.unsafeWindow=(E=I.unsafeWindow)!=null?E:window;var G=I.unsafeWindow;I.GM_info,I.GM_cookie;const se="",S=G,R=(typeof S.jQuery>"u"?S.wrappedJSObject:S).jQuery;class P{constructor(){z(this,"extractedData",{appId:"",name:"",countAllDlcs:0,dlcs:{},countDlcs:0,dlcsUnknowns:{},countDlcsUnknowns:0});z(this,"options",{withDlcsUnknowns:!1});z(this,"is")}check(){this.run();let{href:e}=window.location;n.default("a[href]").on("click",()=>{const{href:t}=window.location;e!==t&&(e=t,this.run(!0))})}run(e=!1){e&&n.default(".sak32009").remove();const{href:t}=window.location,s=new URL(t).searchParams;/https:\/\/steamdb\.info\/app\/\d+\/dlc\//u.test(t)?(this.is="steamdbapp",this.getDataFromSteamDBApp()):/https:\/\/steamdb\.info\/app\/\d+\/depots\//u.test(t)?s.get("branch")==="public"&&(this.is="steamdbacf",this.getDataFromSteamDBForACF()):/https:\/\/steamdb\.info\/depot\/\d+\//u.test(t)?s.has("show_hashes")&&(this.is="steamdbdepot",this.getDataFromSteamDBDepot()):/https:\/\/store\.steampowered\.com\/app\/\d+\/\w+/u.test(t)&&(this.is="steampowered",this.getDataFromSteamPowered())}getDataFromSteamDBApp(){this.extractedData.appId=n.default("div[data-appid]").attr("data-appid"),this.extractedData.name=n.default('h1[itemprop="name"]').text().trim(),n.default("#dlc.tab-pane tr.app[data-appid]").each((e,t)=>{const s=n.default(t),i=s.attr("data-appid"),a=s.find("td:nth-of-type(2)"),c=a.text().trim();a.hasClass("muted")?(this.extractedData.dlcsUnknowns[i]=c,this.extractedData.countDlcsUnknowns+=1):(this.extractedData.dlcs[i]=c,this.extractedData.countDlcs+=1),this.extractedData.countAllDlcs+=1}),this.extractedData.countAllDlcs>0&&this.setModal()}getDataFromSteamPowered(){this.extractedData.appId=n.default("div[data-appid]").attr("data-appid"),this.extractedData.name=n.default("div#appHubAppName").text().trim(),n.default("a.game_area_dlc_row").each((e,t)=>{const s=n.default(t),i=s.attr("data-ds-appid"),a=s.find(".game_area_dlc_name").text().trim();this.extractedData.dlcs[i]=a,this.extractedData.countDlcs+=1,this.extractedData.countAllDlcs+=1}),this.extractedData.countAllDlcs>0&&this.setModal()}getDataFromSteamDBDepot(){const e=n.default("div[data-depotid]").attr("data-depotid"),t=R("#files.tab-pane .table.file-tree").DataTable().data().toArray();let s="";for(const i of t){const a=i,c=a[0],o=a[1];o!=="NULL"&&(s+=`${o} *${c}
`)}s.length>0&&(this.setModal(),this.showOutputOnTextarea(`${e}.sha1`,s))}getDataFromSteamDBForACF(){const e=Number(n.default("div[data-appid]").attr("data-appid")),t=n.default('h1[itemprop="name"]').text().trim(),s=n.default('#config.tab-pane > table td:first-child:contains("installdir")').closest("tr").find("td:last-child").text().trim(),i=Number(n.default('#depots.tab-pane > ul.app-json i:contains("buildid")').closest("li").find("b").text().trim());console.log("appId",e),console.log("appName",t),console.log("appInstallDirectory",s),console.log("appBuildId",i);const a={};a[e]={common:{name:t},config:{installdir:s},depots:{branches:{public:{buildid:i}}}},n.default("#depots.tab-pane > .table-responsive").first().find("tr").each((c,o)=>{const d=n.default(o),p=Number(d.find("td:nth-child(1) a").text().trim()),L=d.find("td:nth-child(2)").text().trim(),r=d.find("td:nth-child(3)").attr("data-sort"),M=typeof r<"u"?Number(r):0,w=d.find("td:nth-child(4)").attr("data-sort"),N=d.find("td:nth-child(5) a").text().trim(),D=d.find("td:nth-child(6)").text();a[e].depots[p]={name:L,maxsize:M},typeof w<"u"&&(a[e].depots[p].config={oslist:w}),N.length>0&&(a[e].depots[p].manifests={public:Number(N)});const y=/DLC (?<dlcid>\d+)/u.exec(D);y!==null&&(a[e].depots[p].dlcappid=Number(y[1]));const g=D.includes("Shared Install");if(g){const h=/Depot from (?<depotid>\d+)/u.exec(D);h!==null&&(a[e].depots[p].sharedinstall=1,a[e].depots[p].depotfromapp=Number(h[1]))}console.log("-------------------------- depotId",p),console.log("depotName",L),console.log("$depotSize",r),console.log("depotSize",M),console.log("depotOs",w),console.log("depotManifestId",N),console.log("depotExtraInfo",D),console.log("depotIsDlc",y),console.log("depotIsSharedInstall",g)}),this.setModal(),this.showOutputOnTextarea(`appmanifest_${e}.acf`,Q(e,a))}setModal(){this.setModalContainer(),this.is!=="steamdbdepot"&&this.is!=="steamdbacf"&&this.setEvents(),this.setModalButton()}setModalButton(){const e=`<div class='sak32009'>
    <button
      type='button'
      ...
      data-bs-target='#${C}'
    >
      <img src='${_}' alt='${x} Main' style='width: 30px; height: auto;' />
      <span class='ms-1'>${x} v${k}</span>
    </button>
  </div>`;n.default(e).appendTo("body")}setModalContainer(){const e=this.is==="steamdbapp",t=this.is==="steamdbdepot",s=this.is==="steamdbacf",i=this.is==="steampowered";let a="";n.default.each(A,(p,L)=>{a+=`<option value='${p}'>${L.name}</option>
`});let c=`<div class='input-group p-1 bg-white border-top border-1 border-sake-secondary'>
    <select id='sake_select' class='form-select border-sake-secondary rounded-0'>
      ${a}
    </select>`;e&&(c+=`<label class='btn btn-outline-sake-secondary' for='sake_unknowns'>
      <div class='form-check'>
        <input class='form-check-input' type='checkbox' id='sake_unknowns' />
        <span>With DLCS Unknowns</span>
      </div>
    </label>`),c+=`<a href='#' id='sake_download' class='btn btn-outline-sake-secondary rounded-0'>Download as file</a>
  </div>
  ...
  <div class='d-flex flex-row justify-content-end p-2 text-bg-sake-secondary'>
    ...
    <div class='me-1'>|</div>`,e&&(c+=`<div class='me-1'>DLCs Unknowns: ${this.extractedData.countDlcsUnknowns}</div>
      <div class='me-1'>|</div>`),c+=`<div>Total DLCs: ${this.extractedData.countAllDlcs}</div>
  </div>`;const o=`<div class='d-flex justify-content-end p-1 bg-white border-top border-1 border-sake-secondary'>
    <a href='#' id='sake_download' class='btn btn-outline-sake-secondary rounded-0'>Download as file</a>
  </div>
  <pre id='sake_output' class='bg-white text-dark p-2 mb-0 border-top border-1 border-sake-secondary'></pre>
  `;let d=`<div class='sak32009'>
      ...
      <div class='modal-dialog modal-dialog-centered modal-lg'>
        <div class='modal-content text-bg-sake-primary'>
          <div class='modal-header flex-column border-0 text-center'>
            <div>
              <img class='modal-header-logo' src='${B}' alt='${x} Author' />
            </div>
            <h5>${x} v${k}</h5>
            <div class='flex-row'>
          ...
          <div class='modal-body p-0'>`;e||i?d+=c:(s||t)&&(d+=o),d+=`</div>
  ...
  </div>`,n.default(d).appendTo("body")}showOutputOnTextarea(e,t){n.default(".sak32009 a#sake_download").attr({download:e,href:this.encodeToDataUri(t)}),n.default(".sak32009 pre#sake_output").html(t).scrollTop(0)}setEvents(){const e=".sak32009 select#sake_select";n.default(e).on("change",t=>{t.preventDefault();const s=n.default(t.currentTarget).find(":selected").val();if(typeof s=="string"){const i=A[s].file,a=i.text,c=this.parse(i.name);let o=this.parse(a);s==="greenLuma2020ManagerBlueAmulet"&&(o=JSON.stringify(JSON.parse(o.replace(/,\]/gu,"]")),void 0,4)),this.showOutputOnTextarea(c,o)}}),n.default(e).trigger("change"),n.default(".sak32009 input#sake_unknowns").on("change",t=>{this.options.withDlcsUnknowns=n.default(t.currentTarget).is(":checked"),n.default(e).trigger("change")})}encodeToDataUri(e){const t=n.default("<textarea>").html(e)[0].value,s=O.default.enc.Utf8.parse(t);return`data:text/plain;charset=utf-8;base64,${O.default.enc.Base64.stringify(s)}`}parse(e){return e=e.replace(/\[dlcs(?: (?<fromZero>fromZero))?(?: prefix="(?<prefix>\d*)")?\](?<content>[\s\S]+?)\[\/dlcs\]/gu,(t,s,i,a)=>{const c=typeof s>"u"?1:0,o=typeof i>"u"?0:Number(i),d=this.options.withDlcsUnknowns?{...this.extractedData.dlcs,...this.extractedData.dlcsUnknowns}:this.extractedData.dlcs;let p=c,L="";for(const r in d)if(Object.prototype.hasOwnProperty.call(d,r)){const M=d[r],w=this.prefixDlcIndex(p.toString(),o);L+=a.replace(/\{(?<content>\w+)\}/gu,(N,D)=>({dlcId:r,dlcIndex:w,dlcName:M})[D]),p+=1}return L}),e=e.replace(/\[data\](?<data>[\s\S]*)\[\/data\]/gu,(t,s)=>this.extractedData[s]),e}prefixDlcIndex(e,t){return t>e.length?"0".repeat(t-e.length)+e:e}}new P().check()})(CryptoJS,$);
