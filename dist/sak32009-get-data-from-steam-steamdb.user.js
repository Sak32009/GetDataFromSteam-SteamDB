...
// @grant        unsafeWindow
// @updatedAT    Thu, 30 Mar 2023 13:42:06 GMT
// ==/UserScript==
...
`;
  ...
  class GetDataFromSteamSteamDB {
    ...
    getDataFromSteamDBApp() {
      this.appInfo.appId = $$1("div[data-appid]").attr("data-appid");
      this.appInfo.appName = $$1('h1[itemprop="name"]').text().trim();
      $$1("#dlc.tab-pane tr.app[data-appid]").each((_, el) => {
        ...
        if (el.hasAttribute("hidden")) {
          this.appInfo.appDLCHiddens[appId2] = appName;
          this.appInfo.appCountDLCHiddens += 1;
        } else if ($appName.hasClass("muted")) {
          this.appInfo.appDLCUnknowns[appId2] = appName;
          this.appInfo.appCountDLCUnknowns += 1;
        } else {
          this.appInfo.appDLC[appId2] = appName;
    ...
    getDataFromSteamDBForACFPrepare() {
      ...
      const appId2 = Number($$1("div[data-appid]").attr("data-appid"));
    ...
    getDataFromSteamDBForACF() {
      ...
      const appId2 = Number($$1("div[data-appid]").attr("data-appid"));
      console.debug(FN_NAME, "appId", appId2);
      const appName = $$1('h1[itemprop="name"]').text().trim();
      console.debug(FN_NAME, "appName", appName);
      const appInstallDirectory = $$1('#config.tab-pane > table td:first-child:contains("installdir")').closest("tr").find("td:last-child").text().trim();
    ...
    setModal() {
      ...
      if (this.appPage !== "steamdbdepot" && this.appPage !== "steamdbacf") {
        this.setEvents();
      }
      this.setModalButton();
    }
    ...
    setEvents() {
      ...
      $sakeSelect.on("change", (event) => {
        ...
        if (typeof selectedOption === "string") {
          const selectedFileInfo = skFormats[selectedOption].file;
